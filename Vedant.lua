...
local C = {
    ...
    bg1         = Color3.fromRGB(0, 0, 0),        -- Header and cards background
    bg2         = Color3.fromRGB(0, 0, 0),        -- Playable lists / list elements
    bg3         = Color3.fromRGB(20, 20, 20),     -- Speed inputs / sliders background
    border      = Color3.fromRGB(120, 120, 120),  -- Inner frame outlines (light gray)
    glassBorder = Color3.fromRGB(255, 255, 255),  -- Outer glow/border (white)
    text        = Color3.fromRGB(255, 255, 255),  -- White body text
    text2       = Color3.fromRGB(220, 220, 220),  -- Light gray text
    text3       = Color3.fromRGB(150, 150, 150),  -- Dim gray text
    accent      = Color3.fromRGB(255, 255, 255),  -- Main accent (white)
    bg4         = Color3.fromRGB(10, 10, 10),     -- Toggle backgrounds
    red         = Color3.fromRGB(255, 255, 255),  -- Close dot (white)
    yellow      = Color3.fromRGB(255, 255, 255),  -- Minimize dot (white)
    green       = Color3.fromRGB(255, 255, 255),  -- Success / Active (white)
...
Menu.BackgroundTransparency = 0.65
...
-- Vedant Theme Background Image (inside Menu window)
local BgImage = Instance.new("ImageLabel")
BgImage.Name = "VedantBgImage"
BgImage.Parent = Menu
...
pcall(function()
    ...
    local imgName = "vedant_bg.png"
    local paths = {}

    if isfile then
        if isfile("workspace/" .. imgName) then table.insert(paths, "workspace/" .. imgName) end
        if isfile(imgName) then table.insert(paths, imgName) end
        if isfile("ReanimData/" .. imgName) then table.insert(paths, "ReanimData/" .. imgName) end
    end

    if #paths == 0 then
        paths = {"workspace/" .. imgName, imgName, "ReanimData/" .. imgName}
    end

    local loaded = false
    for _, imgPath in ipairs(paths) do
        if type(get_asset) == "function" then
            local ok, assetId = pcall(get_asset, imgPath)
            if ok and assetId and tostring(assetId) ~= "" then
    ...
    if not loaded then
        local url = "https://raw.githubusercontent.com/Halo-tuff/qfqsdfqsdfqsdgfhfsdfsg/main/image.png"
        local ok, content = pcall(function()
            return game:HttpGet(url, true)
        end)
        if ok and content and content ~= "" then
            local savePath = "workspace/" .. imgName
            pcall(function() makefolder("workspace") end)
            pcall(function() writefile(savePath, content) end)
            if type(get_asset) == "function" then
                local ok2, assetId = pcall(get_asset, savePath)
                if ok2 and assetId and tostring(assetId) ~= "" then
                    BgImage.Image = assetId
                    loaded = true
                end
            end
    ...
    if not loaded then
        warn("[VedantTheme] Image load failed. Make sure vedant_bg.png is in your executor workspace folder, or you have internet access.")
    end
end)
...
TitleBar.BackgroundTransparency = 0.60; TitleBar.BorderSizePixel = 0
...
toggleBar.BackgroundColor3 = C.bg1; toggleBar.BackgroundTransparency = 0.70
...
npBar.BackgroundColor3 = C.bg1; npBar.BackgroundTransparency = 0.70
...
AnimListFrame.BackgroundColor3 = C.bg0; AnimListFrame.BackgroundTransparency = 0.70
...
BindsPanel.BackgroundColor3 = C.bg0; BindsPanel.BackgroundTransparency = 0.70
...
customListFrame.BackgroundColor3 = C.bg0; customListFrame.BackgroundTransparency = 0.70
...
SettingsPanel.BackgroundColor3 = C.bg0; SettingsPanel.BackgroundTransparency = 0.70
...
SpeedPanel.BackgroundColor3 = C.bg0; SpeedPanel.BackgroundTransparency = 0.70
...
TestingPanel.BackgroundColor3 = C.bg0; TestingPanel.BackgroundTransparency = 0.70
